...
public class Tower extends Structure {
    ...
    private ArrayList<ArrayList<ArrayList<Colors>>> blueprint = new ArrayList<ArrayList<ArrayList<Colors>>>();
    ...
    private void genBlueprint(){
        for (int i = 0; i < x_length; i++) {
            blueprint.add(new ArrayList<ArrayList<Colors>>(x_length));
            for (int j = 0; j < y_length; j++) {
                blueprint.get(i).add(new ArrayList<Colors>(y_length));
    ...
    public String viewBlueprint(){
        ...
        for (int i = 0; i < blueprint.size(); i++) {
            for (int j = 0; j < blueprint.get(i).size(); j++) {
                gb.append(" ["+blueprint.get(i).get(j).size()+"]");
            }
            gb.append("\n");
        }
        ...
        for (int i = 0; i < blueprint.get(0).get(0).size(); i++) {
            ...
            for (int j = 0; j < blueprint.size(); j++) {
                gb.append("["+blueprint.get(0).size()+"]");
            }
        }
        gb.append("\n");
        return gb.toString();
